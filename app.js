const STORAGE_KEY = 'rgrid.webr.workbook.v1';
const PLOT_PANE_WIDTH_KEY = 'rgrid.webr.plotPaneWidth.v1';
...
async function resolveFunctionHelpTarget(token) {
  ...
  const result = await runtime.webR.evalRString(`local({
...
function plotDeviceOptions() {
  return {
    width: Math.max(1, Math.round(plotResolution.width / 2)),
    height: Math.max(1, Math.round(plotResolution.height / 2)),
    bg: 'white',
    capture: true,
  };
}
...
function updatePlotResolutionHelp() {
  ...
  els.plotResolutionHelp.textContent = `Final image: ${candidate.width} × ${candidate.height} px. webR device: ${candidate.width / 2} × ${candidate.height / 2}, rendered at 2× scale.`;
}
...
async function ensurePackagesForExpression(input) {
  ...
  for (const packageName of packages) {
    ...
    let installed = false;
    try {
      installed = await runtime.webR.evalRBoolean(`requireNamespace(${rString(packageName)}, quietly = TRUE)`);
  ...
  try {
    await runtime.webR.installPackages(missing, { quiet: true });
    for (const packageName of missing) {
      const installed = await runtime.webR.evalRBoolean(`requireNamespace(${rString(packageName)}, quietly = TRUE)`);
...
async function recalculateWorkbook() {
  ...
  try {
    await runtime.webR.evalRVoid('.rgrid_cells <- new.env(hash = TRUE, parent = emptyenv())');
    let priorSpills = new Map(spillOwners);
    for (let pass = 0; pass < 3; pass += 1) {
      if (pass > 0) await runtime.webR.evalRVoid('.rgrid_cells <- new.env(hash = TRUE, parent = emptyenv())');
    ...
    setRuntimeStatus(globalThis.crossOriginIsolated ? 'webR ready' : 'webR ready (GitHub Pages mode)', 'ready');
  } catch (error) {
    console.error(error);
...
async function evaluateAndPlaceCell(node) {
  ...
  let shelter;
  try {
    shelter = await new runtime.webR.Shelter();
    const capture = await shelter.captureR(rCode, {
      captureGraphics: typeof OffscreenCanvas === 'undefined' ? false : plotDeviceOptions(),
      captureStreams: true,
...
function parsePackedResult(js) {
  if (!js || js.type !== 'list' || !Array.isArray(js.names)) {
    throw new Error('webR returned an unsupported result.');
  }
...
async function saveNameFromDialog() {
  ...
  try {
    ...
    try {
      if (runtime.ready) await runtime.webR.evalRVoid(`invisible(parse(text = ${rString(expression)}))`);
...
async function exportExcel() {
  ...
  try {
    ...
    setRuntimeStatus(runtime.ready ? 'webR ready' : 'webR unavailable', runtime.ready ? 'ready' : 'error');
  }
}
...
async function initialiseWebR() {
  try {
    ...
    const webROptions = !globalThis.crossOriginIsolated && ChannelType?.PostMessage
      ? { channelType: ChannelType.PostMessage }
      : {};
    runtime.webR = new WebR(webROptions);
    await runtime.webR.init();
    try {
      await runtime.webR.evalRVoid('options(webr.show_menu = FALSE); webr::shim_install()');
    } catch (error) {
      console.warn('Could not enable webR package shims:', error);
    }
    runtime.ready = true;
    els.app.setAttribute('aria-busy', 'false');
    setRuntimeStatus('webR ready', 'ready');
    await recalculateWorkbook();
  } catch (error) {
    runtime.ready = false;
    els.app.setAttribute('aria-busy', 'false');
    setRuntimeStatus('webR failed to load', 'error');
    refreshGridValues();
    showMessage('webR failed to load', `${error.message || error}\n\nServe this directory over HTTP rather than opening index.html through file://. See README.md for commands.`);
  }
}
